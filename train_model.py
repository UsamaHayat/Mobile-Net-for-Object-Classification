...
class DataSequence(Sequence):
    ...
    def __init__(self, csv_file, batch_size=32, inmemory=True, number_of_elements_to_be_output=1):
...
def create_model(size, alpha, number_of_elements_to_be_output):
    ...
    x = Conv2D(77, kernel_size=kernel_size_adapt, name="coords")(x)
    x = Flatten()(x)
    x = Dense(number_of_elements_to_be_output,activation='sigmoid', name='fc' + str(number_of_elements_to_be_output))(x)
    ...
    if number_of_elements_to_be_output == 1:
        rospy.logwarn("CREATED MODEL Classifier Output")
...
def train(model, epochs, batch_size, patience, threads, train_csv, validation_csv, models_weight_checkpoints_folder, logs_folder, model_unique_id, load_weight_starting_file=None, number_of_elements_to_be_output=2, initial_learning_rate=0.0001, min_learning_rate = 1e-8):
    train_datagen = DataSequence(train_csv, batch_size,True, number_of_elements_to_be_output)
    validation_datagen = DataSequence(validation_csv, batch_size, True,number_of_elements_to_be_output)
